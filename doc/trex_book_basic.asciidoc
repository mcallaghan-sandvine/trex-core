...
The following is an example YAML-format traffic configuration file (cap2/dns.yaml), with explanatory notes.
...
<6> Round-trip-time (microseconds): Should be the same as IPG for now.
...
Let's use the `bp-sim` for demonstration purposes:
